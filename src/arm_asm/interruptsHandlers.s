...
swi_handler_int:
	stmfd sp!, {r0-r12}			    // Saving context (r0-r12) in the SVC stack.

	/* Change to SYS Mode to save the SP_USR and LR_USR. */
	mov r0, #0xdf		// (SYS_MODE + NO_INT = 0x1f + 0xc0 = 0xdf)
	msr cpsr_c, r0
	mov r1, sp			// Setting the USER's SP.
	mov r2, lr			// Setting the USER's LR.
	mov r0, #0xd3		// Changing mode to SVC with interrups disable. (SVC_MODE + NO_INT = 0x13 + 0xc0 = 0xd3
	msr cpsr_c, r0
	...
	// lr here is referencing to lr_svc.
