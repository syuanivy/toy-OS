 ...
 * terminated.
 * @param tid specifies the thread we are waiting on.
 */
void thread_wait(tid_t tid) {
    /* we need to disable interrupts here to avoid race condition:
       get the target thread when it's not dead, but when it interrupts
       our else block and become truly dead, the current thread will
       block forever. */
    enum interrupts_level old_level = interrupts_disable();

    // first we need to find the target thread.
    struct thread *target_thread = thread_find(tid);

    // skip cases where target thread does not exist or it is dead..
    if (target_thread != NULL && target_thread->status != THREAD_DYING) {
        // add the current thread into the waiting list of target thread.
...
tid_t thread_create(const char *name, int32_t priority,
    ...
    // initialize waiting list for the current thread.
    list_init(&(thread->waiting_list));
